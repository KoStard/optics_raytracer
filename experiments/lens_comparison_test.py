...
for i in range(iterations):
    ...
    negative_lens = Lens.build(
        center=np.array([5, 5, -((i+1) * step)], dtype=np.float32),  # 2.5m away, offset in y-axis
        radius=4.0,
        normal=np.array([0, 0, -1], dtype=np.float32),
        focal_distance=-10.0  # -1m negative focal distance
    )

    print(f"Lenses created: positive lens (f=10dm) and negative lens (f=-10dm) at {(i+1)*step}dm")
