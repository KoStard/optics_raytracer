...
def create_target_image(size=1000, output_path="target.png", line_thickness=2, circle_thickness=50, step=50):
    """
    ...
    draw.line([(center[0], 0), (center[0], size)], fill='black', width=line_thickness)  # Vertical
    draw.line([(0, center[1]), (size, center[1])], fill='black', width=line_thickness)  # Horizontal
    ...
    while radius <= max_radius:
        ...
        draw.ellipse(
            [(center[0] - radius, center[1] - radius), 
             (center[0] + radius, center[1] + radius)], 
            outline=color, width=circle_thickness
        )
        radius += step
