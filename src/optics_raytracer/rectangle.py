...
class Rectangle:
    ...
    def build(
        ...
        return Rectangle(
            np.array(
                (middle_point, normal, width, height, u_vector), dtype=rectangle_dtype
            )
        )

    @staticmethod
    def get_hits_mask(
        rectangle_array: np.ndarray, points_array: np.ndarray
    ) -> np.ndarray:
        """
        ...
        v = np.cross(rectangle_array["normal"], rectangle_array["u_vector"])[
            :, np.newaxis
        ]
        u_projection_vectors = np.matmul(
            np.matmul(u, u.T) / np.matmul(u.T, u), middle_to_point_vector_array.T
        ).T
        v_projection_vectors = np.matmul(
            np.matmul(v, v.T) / np.matmul(v.T, v), middle_to_point_vector_array.T
        ).T

        return np.logical_and(
            np.linalg.norm(u_projection_vectors, axis=1)
            <= rectangle_array["width"] / 2,
            np.linalg.norm(v_projection_vectors, axis=1)
            <= rectangle_array["height"] / 2,
        )
